...
fun albumAndTrackLowerThanXSeconds(durationInSeconds: Int, albums: List<Album>): List<Pair<String, String>> {
    return albums.asSequence().flatMap {
                val album = it.title
                it.tracks.asSequence()
                        .filter { track -> track.durationInSeconds < durationInSeconds }
                        .map { track -> Pair(album, track.title) }
            }.toList()
}

fun main(args: Array<String>) {
    // List of albums no. 1 in UK
    val titlesTopUK = albums.asSequence()
            .filter { it.chartUK == 1 }
            .map { Pair(it.title, it.year) }
    //.toList()

    listOf(arrayOf(1,2,3), arrayOf(4,5,6)).asSequence()
            .flatMap { it.asList().asSequence() }
            .forEach { println(it) }

    // titlesTopUK.forEach { println(it) }
